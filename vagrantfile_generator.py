...
def generate_ip_pairs(router_connections):
    ...
    for connection in router_connections:
        ...
        if (device1.startswith("r") or device1.startswith("csr")) and (device2.startswith("r") or device2.startswith("csr")):
        ...
        # Switch-Router connections with /24 IP block
            ...
            # Assign IPs from the same /24 block to routers and switches connected to the same switch
            subnet_id = switch_router_map[switch]
            router_ip = f"168.{subnet_id}.0.{len([entry for entry in ip_assignments if entry.get('Device A') == switch or entry.get('Device B') == switch]) + 2}"  # Start from .2
            switch_ip = f"168.{subnet_id}.0.{len([entry for entry in ip_assignments if entry.get('Device A') == switch or entry.get('Device B') == switch]) + 3}"  # Start from .3
